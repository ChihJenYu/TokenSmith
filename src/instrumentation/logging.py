...
class RunLogger:
    ...
    def save_chat_log(
        ...
        timestamp_str = self._filename_timestamp()
        ...
        if not (
            ...
            retrieved_chunks = []
            for i, (chunk, idx, source, score, page_number) in enumerate(
                zip(chunks, top_idxs, sources, ordered_scores, page_numbers_list)
            ):
